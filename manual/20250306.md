...
// function App 빼놓고 복사해왔습니다
  ...
  const [ reposata, setRepodata ] = useState<Repository[]>([]);
```
...
export default App
```
- 이상까지 입력한 이후 return 문에서 입력 필드와 버튼을 구현하고, 입력 필드에 onChange 관련 리스너를 추가하여 입력값을 keyword 상태에 저장할 수 있도록 해야할겁니다. 버튼에는 클릭 리스너가 있고, 주어진 키워드로 REST API를 호출하도록 함수를 작성할겁니다.
...
응답이 items 속을 포함하는 객체라는 점을 확인했고, 그 다음에 items의 속성이 value값이 리포지토리 객체의 배열이라는 점도 볼 수 있었습니다.
...
* 참고 : github API는 요청횟수 제한이 적용돼서 너무 많이 써먹으면 안됩니다, 1분에 10회 요청 제한이 있어서 그 이상 검색하고 나면 1분 기다리셔야 합니다.
