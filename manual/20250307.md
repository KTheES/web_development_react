...
<!-- 저희는 이제 Component 분할 파트로 들어가고 있습니다. -->

- 그리고 이상의 앱은 리액트 쿼리 라이브러리를 이용해서 리포지토리 정보들을 가지고 오고, 리액트 쿼리 내장 기능을 이용하여 요청 상태를 쉽게(useState를 쓸 떄보다) 처리할 수 있었습니다. 리액트 쿼리가 데이터 관리와 캐싱을 처리하기 때문에 응답 데이터를 저장하기 위한 상태(data, setData)가 필요하지 않았습니다.

여러분들의 복습 포인트는 githubapiapp과 gitapi(githubapi입니다!)의 코드 상의 차이점이 되겠습니다.
<!-- 복습 포인트! 20250306 App.jsx와 비교해보기 -->
...
<!-- 리액트와 리액트 쿼리는 다르다 = 자바와 스프링부트가 다른것처럼. 리액트 쿼리만의 문법이 따로 있다. -->
...
const mutation = useMutation({
  ...
  onSuccess: (data, variables, context) => {    // axios.post()에서 추가된 부분입니다.
    // cars 쿼리 무효화
    queryClient.invalidateQueries({ queryKey : ['cars']}); // 이 쿼리 키를 무효화하겠다!
  },
})
```
...
awesome-react-components 라고 검색해보시면
...
저희는 ctrl + f : react-date-picker 사용!

서드파티 컴포넌트 설치 명령어

```cmd
npm install (컴포넌트 이름, 우리는)react-date-picker
```

npm install react-date-picker로 설치하면 가장 최신 버전 설치됩니다.

npm install react-date-picker@4
일 경우에는 저희가 vite project 만들 때처럼 버전을 명시하는 것이 가능합니다.

삭제 명령어

```cmd
npm uninstall 컴포넌트이름
```
...
그리고 다양한 ag grid 관련 css중에 material 디자인 스타일 적용을 하기 위해
...
```
를 import 했습니다. 다른 css 파일들의 경우에는 node-modules 폴더(의존성을 모아둔)에 ag-grid-community를 확인하시면 됩니다.
